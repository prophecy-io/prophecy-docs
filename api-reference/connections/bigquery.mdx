---
title: BigQuery
description: Properties required to create and update BigQuery connections
---
...
<Tip>
  Create the secret containing your service account key JSON in Prophecy before creating the
  BigQuery connection. The secret must be of type `text` and contain the full service account key
  JSON as its value.
</Tip>
...
<ParamField body="dataset" type="string" required>
  Default dataset name to use for queries when no dataset is explicitly specified.
</ParamField>
