...
1. Open your pipeline in [Studio](/data-analysis/development/studio/studio).
1. Click **Source/Target** in the canvas.
1. Select **Source** or **Target** from the dropdown.
1. Click the gem to configure it.
...
In the **Location** tab, set your file format and connection details. To learn more, see [Location](#location) below.

</Step>

<Step title="Set file properties">
In the **Properties** tab, set file properties. These vary based on the file type that you are using.
...
2. Take the site's normal URL (such as `https://prophecy.sharepoint.com/sites/testsite`) and convert it to the Graph format: replace the first `/` after the hostname with a colon (`:`).
...
<Warning>
  The hostname and site path need to be joined with a colon, not a slash.
