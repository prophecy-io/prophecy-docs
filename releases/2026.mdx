...
Additional performance improvements reduce the amount of gem specification data sent to the browser, speed up SQL parsing during validation and compilation, and use the faster dbt Fusion engine to for dbt-backed projects.
...
To pick up these improvements, re-import existing Informatica estates.
...
The [Pivot gem](/data-analysis/gems/transform/pivot) also now has a **Simple** mode. Instead of manually typing pivot column values, click **Fetch unique values** to retrieve and select the distinct values from your pivot column directly. Column aliasing for aggregations is available on Databricks and BigQuery, but not on Snowflake.

</Update>

<Update label="Prophecy 4.2.11" description="September 4, 2026" tags={["Administration"]}>
