...
<Update label="Prophecy 4.2.11" description="September 4, 2026" tags={["Data Analysis"]}>

## Large SQL projects load and compile faster

Large SQL projects now open and compile faster. Prophecy reuses cached results for unchanged pipelines instead of recompiling them each time. In testing with a 13,000-node project, load time decreased from 46 seconds to 25 seconds.

Additional performance improvements reduce the amount of gem specification data sent to the browser, speed up SQL parsing during validation and compilation, and use the faster dbt Fusion engine to initialize dbt-backed projects. Undo and redo behavior is also now consistent across the product.

## Informatica-to-BigQuery conversion fidelity

Informatica-to-BigQuery conversions now more closely preserve Informatica behavior across joins, lookups, aggregations, routers, sequence generators, date and string functions, file and target handling, connections, and parameters.

The improvements focus on runtime fidelity, including NULL handling, safer date parsing and casts, deterministic sequence generation, and preservation of file formats and physical table names.

Re-import existing Informatica estates to pick up these improvements.

## Large projects load and compile faster

Large SQL projects now open and compile faster. Prophecy caches unchanged pipelines instead of recompiling them each time. In testing with a 13,000-node project, load time decreased from 46 seconds to 25 seconds.

Additional improvements speed up SQL parsing and dbt project initialization while reducing unnecessary data sent to the browser. Undo and redo behavior is also now consistent across the product.
...
See [Gems for Data Analysis](/data-analysis/gems/gems#add-a-gem-from-a-connection) for details.

## Visual containers: new actions and background color

[Visual containers](/data-analysis/development/studio/containers) have a refreshed action menu. Grouping gems now happens via **Actions > Group** at the bottom of the canvas, and each container's **...** menu adds **Add to AI**, **Explain**, and **Edit Background Color**, alongside the existing Ungroup, Attach Annotation, and Collapse/Expand options.

## Canvas annotations

Added an annotate button at the bottom left of the canvas for adding free-form text annotations directly on the pipeline. See [Canvas annotations](/data-analysis/development/studio/canvas-annotations).

## Alt-key row counts and wireless connection reveal

Holding **Alt** (**Option** on Mac) on the canvas now switches interim row-count labels to their exact value instead of a rounded, compact format (for example, `100K rows` becomes `100,000 rows`). Holding **Alt** also reveals all [wireless connections](/data-analysis/development/studio/wireless-connections) on the canvas at once, rather than only the one currently selected. Releasing **Alt** restores the compact display and hides unselected wireless connections again.

## Mark pipelines as draft

You can now mark an individual pipeline as `draft` to keep working on it while excluding its in-progress changes from the project's next published version. If the pipeline has been published before, it ships unchanged in the new version; if it hasn't, it's left out entirely. Existing schedules on a draft pipeline keep running, and the pipeline can't be renamed or deleted while marked as draft.

Toggle draft status from the **...** menu in the pipeline canvas header (**Keep as draft** / **Remove draft**). Removing draft status doesn't publish the pipeline immediately — it's picked up the next time the project is published. The changes list during publication shows held-back pipelines and apps with a **Draft - Held** label and a summary count.

See [Mark pipeline as draft](/data-analysis/development/pipelines/data-analysis-pipelines#mark-pipeline-as-draft) and [Draft pipelines](/data-analysis/production/publication/publication-concepts#draft-pipelines).


## Lock Pipeline gem triggers and schedules to a specific version

The [Pipeline gem](/data-analysis/production/scheduling/pipeline-trigger-gem#version) now has a **Version** parameter for choosing which published version of the target pipeline to trigger: **Current** (default, always up to date), **Last Released** (resolves to the newest published version at run time), or a specific version number, which keeps triggering that exact release even as the target pipeline is edited, drafted, or republished.

Similarly, [schedules](/data-analysis/production/scheduling/schedule-setup#lock-a-schedule-to-a-version) can now be locked to a specific published version instead of always following the latest publish. Locks apply per fabric, so the same schedule can be locked on one fabric's deployment while following the latest published version on another.

## Improved Excel file uploads

Uploading Excel files (XLS, XLSX) via [Upload files in Agent chat](/data-analysis/ai/agent/chat/upload-files#excel-specific-properties) or the [Upload Files gem](/data-analysis/gems/source-target/table/upload-files#excel-specific-properties) now exposes Excel-specific read options.

You can also mark multiple regions of a workbook as separate tables. In the Upload Files gem, each marked table becomes its own output port, with independent schema editing, drift-handling, and the ability to unmark or delete it.

The preview grid for Excel uploads now loads additional rows and columns as you scroll instead of fetching the entire sheet at once, and handles merged cells and blank rows in the source sheet automatically.

## Join gem: Simple and Advanced modes

The [Join gem](/data-analysis/gems/join-split/join) now offers a guided **Simple mode** alongside the existing **Advanced mode**. Simple mode walks you through choosing a join type, defining match conditions, and selecting output columns for two-input joins; a **Where rows go** panel shows which output port each row lands on for the selected join type.

Simple mode also adds **Semi Join** and **Anti Join** types (Full Outer, Cross, Natural, and Semi/Anti joins remain Advanced-mode-only from the dropdown), and the gem now always exposes reject ports alongside the matched output:

If a join's configuration is too complex for Simple mode, the gem prompts you to either switch to Advanced mode or reset to a default inner join.

## Pivot gem: warehouse-specific configuration and unique value fetching

The [Pivot gem](/data-analysis/gems/transform/pivot) configuration is now organized into Snowflake, Databricks, and BigQuery tabs, each with **Grouping**, **Pivot**, and aggregation sections. Instead of manually typing pivot column values, click **Fetch unique values** to retrieve and select the distinct values from your pivot column directly. Column aliasing for aggregations is available on Databricks and BigQuery, but not on Snowflake.

</Update>

<Update label="Prophecy 4.2.11" description="TBD" tags={["Administration"]}>

## Execution plane events in audit logs

[Audit logs](/administration/management/audit-logs#execution-plane-events) now capture actions performed directly on the execution plane (the orchestrator service running in your environment) — such as fabric, connection, secret, deployment, pipeline run, and schedule operations — in addition to control plane GraphQL calls and UI interactions. These events are recorded locally on the execution plane first, then forwarded to the control plane in batches (every 30 minutes by default), so there's a short delay before they appear in the central audit log.

Execution plane events are identified by a fixed event type and category (for example, `pipeline_run_scheduled` / `compute`, `connection_create` / `data_flow`, `login_federated` / `auth`) rather than a GraphQL query name.
...
The Table gem now lets you reconcile schema differences when writing to an existing target table.

You can use Map Schema to assign each target column its corresponding source column. Prophecy suggests mappings automatically for unmapped columns, which you can review and adjust, and applies any required casts or transformations so incoming data conforms to the target schema.

You can also use Overwrite Target Schema if you'd rather the target adopt the incoming schema instead; this replaces the existing target schema with the source schema.

These enhancements are available for Table targets across [Databricks](/data-analysis/gems/source-target/table/databricks-write), [BigQuery](/data-analysis/gems/source-target/table/bigquery-write), and [Snowflake](/data-analysis/gems/source-target/table/snowflake-write), making it easier to write into existing tables without manually reconciling schema drift.
...
Prophecy now supports BigQuery's native `JSON` data type for both [reading from](/data-analysis/gems/source-target/table/bigquery-read#working-with-json-columns) and [writing to](/data-analysis/gems/source-target/table/bigquery-write#working-with-json-columns) BigQuery.
