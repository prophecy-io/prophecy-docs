...
[Visual containers](/data-analysis/development/studio/containers) get a refreshed action menu. Grouping gems now happens via **Actions > Group** at the bottom of the canvas, and each container's **⋯** menu adds **Add to AI**, **Explain**, and **Edit Background Color**, alongside the existing Ungroup, Attach Annotation, and Collapse/Expand options.
...
<Note>
Draft pipelines are available only for projects that use Simple Version Control.
</Note>
...
| Field | Purpose |
| ----- | ------- |
| **Read mode** | Read a single sheet, or a union of multiple sheets combined into one table. |
| **Sheet Name** | Which sheet(s) to read from. |
...
| **Allow Undefined Rows / Allow Incomplete Rows** | Control whether rows outside the detected schema, or missing values in some columns, are included. |
