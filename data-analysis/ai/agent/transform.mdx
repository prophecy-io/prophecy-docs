...
You can use the Prophecy Agent to generate transformations based on natural language prompts. When you describe a data operation, the Agent adds the corresponding gems to the pipeline and provides a summary of the changes in the chat interface.
...
The Transform Agent operates within a [project](/data-analysis/development/projects/create-project). It can:
...
The Transform Agent may retrieve metadata and limited data samples to validate transformations and ensure correctness.

When executing SQL or running pipelines, the Agent operates under the permissions of the user who invoked it. Existing warehouse access controls and governance policies are always enforced.
...
All changes made by the Agent are saved in the project [version history](/data-analysis/development/versioning/version-control). Commits will be clearly marked as authored by the Agent.
