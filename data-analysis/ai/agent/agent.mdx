...
The Prophecy Agent uses natural language to interact with your data warehouse and help you build pipelines. Describe what you need, and the Agent searches metadata, retrieves sample data, and generates transformations within your project. You can build pipelines incrementally or begin from a single prompt, then inspect and refine each step in the editor.

## Why use the Agent?

The Agent accelerates pipeline development by eliminating manual steps in the development workflow. Instead of browsing schema lists, manually configuring gems, or writing documentation from scratch, you describe your intent and the Agent generates the implementation within your project. This reduces switching between tools, decreases the time spent on repetitive configuration tasks, and allows you to focus on business logic.
...
The Prophecy Agent is built on a specialized version of Claude Code and operates directly within your active Prophecy project. It combines large language model reasoning with structured access to your project assets, metadata, and connected data fabric.

Unlike a standalone chatbot, the Agent is connected to Prophecy's knowledge graph and development environment. This allows it to understand:
...
The Agent does not generate text alone. It uses a set of internal tools to take concrete actions inside your project, including:
...
The Agent operates strictly within the currently active project.
...
- Interact with external Prophecy APIs outside the project scope.

All Agent activity remains contained within the project where it is invoked.
...
The Agent proposes and applies changes inside your project, but you remain in control. You can:
...
The Agent accelerates development, but it does not replace review. Every change remains visible, versioned, and fully editable within Prophecy.
...
<Accordion title="Transform data automatically to match a defined schema">
  Create a Common Data Model (CDM) and use the Harmonization Agent make input data conform to the
  CDM. Learn more in the [Harmonization](/data-analysis/ai/agent/harmonization/overview)
  documentation.
</Accordion>
