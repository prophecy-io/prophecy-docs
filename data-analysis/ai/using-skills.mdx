...
4. Enter a [description](#description) and [content](#content). Changes save automatically.
...
2. In the **Add Skill** dialog, enter a skill name. By default the skill is created in the `/skills` folder.
3. Click **Create**. The skill editor opens.
4. Enter a [description](#description) (1) and [content](#content). Changes save automatically.
...
Here are some effective prompts for improving Prophecy skills:

### 1. Start with discovery
> "Look at [skill name] skill. How can I improve this so that it works better for Prophecy?"

This triggers an analysis of the current state and generates specific recommendations before making changes.

### 2. Request data integration
> "Make this skill use actual data from the workspace instead of just asking questions"

Transforms Q&A-only skills into data-driven tools that query tables before conversing.

### 3. Add pipeline generation
> "Update the skill to offer creating pipelines when appropriate"

Skills become more useful when they can build artifacts, not just give advice.

### 4. Add Analysis/Dashboard output
> "Make the skill create Analysis dashboards to visualize recommendations"

Turns text recommendations into interactive visual artifacts.

### 5. Connect to Existing Work
