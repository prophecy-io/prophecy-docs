...
BigQuery does not allow certain special charactßers in column names. When a pipeline uses a BigQuery fabric, Prophecy automatically encodes column names that contain unsupported characters for storage in BigQuery, then decodes them back to their original form for display throughout the pipeline UI.

For example, a column named `revenue(USD)` is stored in BigQuery as `_PCYBQ__revenue_lp_USD_rp_`. In schema panels, the expression builder, and data previews, Prophecy displays the original name. You can hover over an encoded column name to see the physical name stored in BigQuery.

The following characters trigger encoding: `!`, `"`, `$`, `(`, `)`, `*`, `,`, `.`, `/`, `;`, `?`, `@`, `[`, `\`, `]`, `^`, `` ` ``, `{`, `}`, `~`

<Note>
  This encoding is handled automatically. You do not need to rename columns before using a BigQuery fabric.
</Note>
