...
Microsoft recommends the `Sites.Selected` permission over tenant-wide access, since it scopes the service principal to specific site collections only.

<Info>
  Adding `Sites.Selected` in Azure alone does not grant access to any site. You must also complete
...
2. In the **Request body** tab, paste the following, replacing `<app-client-id>` and `<app-client-id>` with your service principal's values:
