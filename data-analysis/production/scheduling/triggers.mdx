...
<Info>
  When using time-based triggers, the default timezone is the timezone from where you access
  Prophecy.
</Info>
...
<Info>
  Prophecy supports file change detection (data change sensors) for
  [S3](/data-analysis/environment/connections/s3) and
  [SFTP](/data-analysis/environment/connections/sftp) connections only.
</Info>
...
You can define multiple trigger configurations to monitor more than one location for file updates. When you add more than one configuration, the pipeline uses `AND` logic, whereby all conditions must be met before the pipeline runs.
...
With this setup, the pipeline runs only when a change is detected for both Transactions and Exchange Rates.
