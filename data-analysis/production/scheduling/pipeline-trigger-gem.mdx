...
|---|---|
| **Sequential execution** | When a Pipeline gem triggers a target pipeline multiple times, pipeline instances run sequentially. The Pipeline gem does not finish until all triggered pipeline instances complete. |
| **Sequential triggering** | Pipeline gems wait for upstream pipelines to finish when multiple Pipeline gems are configured sequentially or assigned sequential [gem phases](/data-analysis/gems/#gem-phase). |
| **Parallel triggering** | Pipeline gems can execute in parallel when separate pipeline branches share the same gem phase. |
| **Recursive triggers** | Recursive execution is supported. If a triggered pipeline contains another Pipeline gem, that Pipeline gem executes as expected. |
