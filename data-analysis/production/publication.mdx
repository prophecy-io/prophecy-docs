...
Next, the project is deployed to different environments. The deployment process builds the project in each target [fabric](/data-analysis/environment/fabrics/prophecy-fabrics) (environment) and enables pipeline [schedules](/data-analysis/production/scheduling/scheduling) for each fabric.

<Info>
  If you publish a project without selecting a fabric, it is only **released**. This means a new
  version is created and made available for use in the Package Hub, but no deployments are created.
</Info>
...
{/* TODO: Confirm whether Publish remains a direct project header action or moves into the project settings menu. */}
...
All of your teams' published projects will appear in the [Observability](/data-analysis/production/monitoring) interface in Prophecy. This lets you see information like:
