...
Analysis dashboards enable teams to create interactive and reusable workflows that simplify data processing. These applications allow users to parameterize pipelines, interact with data, and schedule pipeline runs without modifying the underlying pipeline logic.

When creating a Prophecy App, structure it using [pipeline parameters](/data-analysis/development/pipeline-params). Pipeline parameters allow applications to dynamically adjust values during execution. By adding [interactive components](/data-analysis/production/analysis/analysis-components) to your apps, users can define parameter values while keeping the rest of the pipeline unchanged.
...
<Note>This tutorial uses sample data provided by Databricks.</Note>
...
[Pipeline parameters](/data-analysis/development/pipeline-params) enable dynamic behavior in pipelines by allowing values to be set at runtime. In this case, these values can be driven by inputs from external sources, including user input through a Prophecy App.
...
1. Enter `3000007` as the default value to be used during [interactive pipeline runs](/data-analysis/development/runs/execution). (This is an arbitrary franchise ID.)
1. Click **Save**.

![Create pipeline parameter dialog](/data-analysis/production/analysis/img/pipeline-parameter.png)
...
<Tip>
  Find your configured pipeline parameters by clicking on the **Configuration Variable** in the
...
1. Select the pipeline parameter set you want to use for this app by default. If you don't define a parameter set for an app, the active parameter set is used.
...
![Prophecy App template](/data-analysis/production/analysis/img/prophecy-app-template.png)
...
To explore advanced configuration options and scheduling capabilities, refer to the documentation on [running apps](/data-analysis/production/analysis/consume-analysis).

<Warning>
  When you run a Prophecy App, the entire underlying pipeline is executed—not just the components
  visible in the app. This means any additional transformations, tables, or other components
  included in the pipeline will also run, even if they're not exposed in the app interface. Be
  mindful of how you design the pipeline to ensure app users only trigger the intended logic.
</Warning>

## What's next

To address your specific business requirements, leverage more complex [components](/data-analysis/production/analysis/analysis-components) to construct robust Prophecy Apps.
