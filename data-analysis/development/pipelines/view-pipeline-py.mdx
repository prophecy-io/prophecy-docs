...
The following snipped represents a pipeline that runs a transformation (`sales_by_region`) and then sends the results via email. The connection `transform >> email` shows that the email step depends on the transformation output. See [classes](#classes) for an explanation of classes.
...
| **`Process`**          | A step in the pipeline (may combine multiple gems)     | Each process is one operation (such as `Transform`, `Visualize`, or  `Email`).<br/><br/> `name` identifies the step- `properties` defines what the step does. |
