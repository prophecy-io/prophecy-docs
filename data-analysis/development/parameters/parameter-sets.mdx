...
You can select a parameter set when [creating a Prophecy App](data-analysis/collaboration/prophecy-apps/app-creation) or modify it afterward in [app settings](data-analysis/collaboration/prophecy-apps/app-settings). This determines which pipeline parameter values the app uses when it runs. On-demand app runs always use values from the active project parameter set.
...
When you [schedule a Prophecy App](data-analysis/collaboration/prophecy-apps/run-apps), you can choose which pipeline parameter set to use for that schedule. This lets you run the same app with different configurations on different schedules.
