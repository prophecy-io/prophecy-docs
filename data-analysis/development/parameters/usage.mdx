...
description: Reference parameters in pipelines, gems, and Prophecy Apps
---
...
- Enabling user-entered fields in [Prophecy Apps](/data-analysis/collaboration/prophecy-apps/apps).
- Keeping values consistent throughout the pipeline.
- Configuring pipelines for multiple execution environments such as development or production. Use app configs or the [Deploy Project API](/api-reference/projects/deploy-project) to do so.
...
Project parameters are shared across all pipelines in a project. Use project parameters when you need variables that can be reused across multiple pipelines.

<Info>
  If a pipeline parameter and a project parameter share the same name, the value of the pipeline
  parameter takes precedence within that pipeline.
</Info>
...
Prophecy supports the following data types for parameters:
