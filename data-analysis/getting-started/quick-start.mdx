...
<Note>
  ...
  one](/data-analysis/environment/fabrics/create-fabric).
</Note>
...
1. Give your project a **Name**, such as `Prophecy_Quickstart`.
...
Prophecy will open the your new project in the [Studio](/data-analysis/development/studio/studio).
...
1. For the **Pipeline Name**, enter `patient_analytics`.
...
1. Click **Table**. This adds a new [Table gem](/data-analysis/gems/source-target/source-target) to the canvas.
...
To materialize the seed data into the SQL warehouse, run the pipeline once.
...
Locate and open the **Chat** tab in the left sidebar.
...
1. Close the preview dialog to return to the chat.
