...
In Prophecy, this logic is implemented through a Join gem (to attach lookup entries) and a Reformat gem that applies SQL expressions such as `regexp_replace` or structured substitutions.

<Note>
For direct, single-pattern regular expression replacements that do not require a lookup table, you can use [Prophecy’s Regex gem](https://docs.prophecy.ai/data-analysis/gems/parse/regex#replace-configuration).
</Note>
