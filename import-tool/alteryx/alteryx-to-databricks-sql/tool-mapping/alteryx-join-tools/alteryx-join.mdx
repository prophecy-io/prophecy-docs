...
Prophecy implements this functionality through the [Join gem](/data-analysis/gems/join-split/join), which joins two or more datasets at a time using explicit join conditions and a selectable join type.
...
- For each input dataset, add a [RecordID gem](/data-analysis/gems/prepare/record-id), which assigns a unique identifier to each row of a dataset, configured so that the Output Column Name is `recordPositionforJoin`.
