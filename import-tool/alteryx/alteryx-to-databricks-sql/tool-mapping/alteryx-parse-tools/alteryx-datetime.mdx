...
In Prophecy, the same outcomes are typically implemented in a [Reformat gem](/data-analysis/gems/prepare/reformat) using `TO_TIMESTAMP` and `DATE_FORMAT`.
...
- In a subsequent Reformat gem, Prophecy either
  ...
  - uses [`to_timestamp`](https://docs.databricks.com/aws/en/sql/language-manual/functions/to_timestamp) (optionally followed by `date_format`) to convertin string values into formatted date/time values.
...
2. Add a **target column** (or select an existing column to overwrite).
