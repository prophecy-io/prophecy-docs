...
When Import detects an Alteryx Count Records tool, it generates a [Reformat gem](/data-analysis/gems/prepare/reformat) that computes a row count using an expression such as `count(1)`. (This expression counts all input rows and preserves the semantics of the Alteryx Count Records tool.)

The resulting dataset contains a single row with the total record count.
...
This produces a single-row output containing the total number of records in the input dataset.
