...
1. Configure **Order by** column(s) to define the row sequence.
1. Configure **Partition by** column(s) if the running total should reset by group.
1. Configure a target column and use a `sum` expression for the running total.
