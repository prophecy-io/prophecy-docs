...
Prophecy implements similar logic through the [Join gem](/data-analysis/gems/join-split/join), which supports multiple inputs and produces a single output dataset.
...
- For each input dataset, add a [RecordID gem](/data-analysis/gems/prepare/record-id) configured to generate a positional index, such as `recordPositionForJoin`.
