...
- Lookup-table replacements are migrated to a **Join gem** followed by a transformation that performs the find/replace logic.
  - For imports into Databricks SQL, this logic is typically implemented in a **Reformat gem**.
  - For imports into BigQuery environments, Import may instead generate a **SQL Statement gem** that expresses the replace logic as a SQL query including all columns.
...
<Note>
When importing workflows for **BigQuery**, some Replace expressions cannot be represented directly in a Reformat gem. In these cases Import generates a **SQL Statement gem** containing the equivalent SQL transformation. This ensures that the replacement logic operates correctly across all columns in the dataset.
</Note>
...
In BigQuery environments, the transformation may appear as a **SQL Statement gem** instead of a Reformat gem when the replace logic must be expressed as a full SQL query.
...
- When importing workflows for **BigQuery**, some Find Replace operations are implemented using a **SQL Statement gem** instead of a Reformat gem.
