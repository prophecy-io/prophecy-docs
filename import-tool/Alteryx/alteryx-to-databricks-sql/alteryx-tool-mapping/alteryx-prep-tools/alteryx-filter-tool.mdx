...
The [Alteryx Filter tool](https://help.alteryx.com/current/en/designer/tools/preparation/filter-tool.html) and the [Prophecy Filter gem](https://docs.prophecy.io/analysts/filter) perform the same essential task: selecting rows that satisfy a logical condition. Both evaluate a Boolean expression against each incoming record.
