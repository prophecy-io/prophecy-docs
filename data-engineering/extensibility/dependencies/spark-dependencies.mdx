...
<Info>
  In rare cases, your dependency might be only accessible to the Spark cluster or the build system,
  but not to Prophecy itself. If you're confident that the dependency is correct, but the validation
  error shows up, it's safe to press **Save Anyways** to ignore that warning.
</Info>
...
![Update dependencies](/data-engineering/extensibility/dependencies/img/update-all-dependencies.png)

<Warning>
  Connecting a Prophecy project to a Spark cluster with a different dependency version will prompt a
  cluster restart.
</Warning>
...
![Update Spark cluster](/data-engineering/extensibility/dependencies/img/synth_0_1_requirements.png)

### Scala JAR selection for Spark 3.x and Spark 4.x

Scala pipeline projects generate different JAR names for Spark 3.x and Spark 4.x runtimes. This allows Prophecy to automatically select the correct JAR based on the target cluster runtime.

Spark 4.0 (Databricks Runtime 17+, Dataproc 3.x) uses Scala 2.13 instead of Scala 2.12. These versions are binary-incompatible, so a JAR compiled for one cannot run on the other and may produce errors such as `NoClassDefFoundError: scala/Serializable`.

To support both runtimes, Prophecy builds version-specific JARs from the same pipeline project using Maven profiles.

#### JAR naming

JAR names now include the Scala version:

| Before | After |
| --- | --- |
| `MyPipeline-1.0.jar` | `MyPipeline_2.12-1.0.jar` (Spark 3.x) |
| — | `MyPipeline_2.13-1.0.jar` (Spark 4.x) |

Prophecy automatically selects the appropriate JAR at runtime.

#### Maven profiles

The project `pom.xml` includes profiles for each Scala version:

| Profile ID | Scala Version | Spark Version | Active by Default |
| --- | --- | --- | --- |
| `scala-2.12` | `2.12.15` | `3.5.0` | Yes |
| `scala-2.13` | `2.13.14` | `4.0.0` | No |

The `scala-2.12` profile remains active by default for existing Spark 3.x workloads.

#### Required actions

This applies only to Scala pipeline projects.

**All Scala pipeline users**:
- Open each pipeline once in the Prophecy UI to trigger code regeneration.
- Confirm that the `pom.xml` file includes Maven profiles.

**Cross-project dependencies**: Re-release upstream projects so downstream projects resolve the updated JAR names.

**Standalone projects**: No action is required.

Use `scala-2.12` for Spark 3.x runtimes and `scala-2.13` for Spark 4.x runtimes.
...
![Storage code for Scala and Python](/data-engineering/extensibility/dependencies/img/dependencies-storage.png)
...
<Info>
  For any help required to enable the template for older projects, please reach out to the Prophecy
  support team.
</Info>
...
<Info>
  Special consideration for Scala dependencies in Pyspark projects must be given when deploying WHL
  files outside of PBT. If you do not create jobs in the Prophecy editor or use the `pbt deploy` or
  `pbt deploy-v2` commands, then this section will help track those Scala dependencies.
</Info>
...
You can use the following option in [Prophecy Build Tool (PBT)](/data-engineering/ci-cd/prophecy-build-tool/prophecy-build-tool) to
