...
[Gems](/data-engineering/gems/gems) are functional units in a [pipeline](/data-engineering/development/pipelines/pipelines) that perform tasks such as reading, transforming, writing, or handling other data operations.
...
| ---------------------------------------------------- | ----------------------------------------------------------------------------------------------------------- |
| [**Source and Target**](/data-analytics/gems/source-target/source-target)    | The set of gems that help with loading and saving data.                                                     |
| [**Transform**](/data-engineering/gems/transform)                | The set of gems that help with transforming data.                                                           |
| [**Join and Split**](/data-engineering/gems/join-split)          | The set of gems that help with the process of merging or splitting DataFrame(s) to create new DataFrame(s). |
| [**Custom**](/data-engineering/custom/)                     | The set of gems that our creative teams build using Expression Builder to extend the Prophecy capabilities. |
| [**Machine Learning**](/data-engineering/machine-learning/) | The set of gems that prepare data or use data for Machine Learning.                                         |
| [**Subgraph**](/data-engineering/gems/subgraph/subgraph)                  | A gem that can contain many other gems within it.                                                           |
...
|  7  | Unit tests         | A set of unit tests. See [here](/data-engineering/ci-cd/tests) for more details.                                                         |

## Input ports

The Input tab defines the incoming connections accepted by the gem. Most gem types only accept one connection, but some (such as [Join](/data-engineering/gems/join-split/join)) allow for multiple inputs.
...
| **Add Condition** | Write a [condition](/data-engineering/development/runs/conditional-execution) that will restrict gem execution during pipeline runs.                                                                                                     |
...
| **Data Preview**  | When [selective data sampling](/data-engineering/development/runs/data-sampling) is enabled, use this checkbox to enable or disable the generation of the data sample for the gem.                                                       |
| **Data Diff**     | When [Data Diff](/data-engineering/ci-cd/data-diff) is configured for a Target gem, use this checkbox to enable or disable the data diff output.                                                                                           |
