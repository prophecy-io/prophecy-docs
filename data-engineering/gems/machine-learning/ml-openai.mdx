...
Follow along to learn how to interact with the OpenAI API using Prophecy's easy-to-use interface. For an example set of pipelines that use these gems to create a Generative AI Chatbot, see this [guide.](/data-engineering/generative-ai-chatbot)
...
| Context | string - a text corpus related to the question of interest, also included in the prompt sent to OpenAI. Frequently the context column should undergo data transformations in the gems preceding the OpenAI gem. See [this guide](/data-engineering/generative-ai-chatbot) for a great example of preparing the text corpus and transforming sufficiently to include in a useful prompt. | False |
