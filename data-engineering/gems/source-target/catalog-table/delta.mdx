...
| merge      | Use the Delta `merge` command to insert, delete and update data. For more information, see [Delta MERGE](/data-engineering/gems/source-target/file/delta#delta-merge). |
| scd2 merge | Store and manage the current and historical data over time. For more information, see [Delta MERGE](/data-engineering/gems/source-target/file/delta#delta-merge).      |

<Tip>
  These `overwrite`, `append`, `ignore`, and `error` write modes operate the same way as with other
  native Spark-supported formats such as Parquet.
</Tip>
