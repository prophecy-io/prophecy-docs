...
For Spark 3.3, [connect a dependency](/data-engineering/extensibility/dependencies/spark-dependencies) that has the following Maven coordinates on your Databricks or OnPrem Execution Cluster:
...
<div style={{ position: 'relative', 'padding-bottom': '56.25%', height: 0 }}>
  <iframe
    src="https://www.loom.com/embed/f5c150ff8a1b4c1fb1485c20e08fde50"
    frameborder="0"
...
| Throughput Control Preferred Regions List                  | Comma-separated list of regions you prefer to use for a multi-region CosmosDB account. Use a collocated Spark cluster with your CosmosDB account and pass the Spark cluster region as a preferred region. To see a list of Azure regions, see [LocationNames Class](https://learn.microsoft.com/en-gb/dotnet/api/microsoft.azure.documents.locationnames?view=azure-dotnet&preserve-view=true). | None    |
| Disable TCP connection endpoint Rediscovery                | Whether to disable TCP connection endpoint rediscovery. You should disable TCP connection endpoint rediscovery when using custom domain names with private endpoints in a custom Spark environment. When using Azure Databricks or Azure Synapse as Spark runtime it should never be required to disable endpoint rediscovery.                                                                  | false   |
| Use Gateway Mode                                           | Whether to use gateway mode for the client operations.                                                                                                                                                                                                                                                                                                                                          | false   |
...
| ---------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------- |
| Default Patch Operation Type | Default CosmosDB patch operation type. Possible values are: `None`, `Add`, `Set`, `Replace`, `Remove`, or `Increment`. Choose `none` for no operation, and for the rest, see [Supported operations](https://learn.microsoft.com/en-gb/azure/cosmos-db/partial-document-update#supported-operations) to determine which one to select. | `Replace`     |
