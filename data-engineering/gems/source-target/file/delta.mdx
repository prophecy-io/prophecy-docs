...
<Note>You can only select `Read Timestamp` or `Read Version`, not both.</Note>

### Example

![Delta source example](/data-engineering/gems/source-target/file/img/delta/delta_source_eg.gif)
...
![Delta Target Example](/data-engineering/gems/source-target/file/img/delta/delta_target_eg.gif)
...
| When Matched Update Expressions | Expressions for setting the values of columns that the Target gem needs to update. <br /><br/>If you add some expressions, enable the **Update Remaining Columns from Source** checkbox to also update all remaining columns from the source `DataFrame`.                                                                                                                                                                                      | None     |
| When Matched Delete Action      | Delete rows if your `When Matched Delete Condition` property and the optional additional condition evaluates to `true`. <br /><br />Delete removes the data from the latest version of the Delta table but does not remove it from the physical storage until you explicitily vacuum the old versions. To learn more, see [Remove files no longer referenced by a Delta table](https://docs.delta.io/latest/delta-utility.html#-delta-vacuum). | `ignore` |
...
<Tip>
  When possible, provide predicates on the partition columns for a partitioned Delta table because
  predicates can significantly speed up the operations.
</Tip>
...
![Initial customer table](/data-engineering/gems/source-target/file/img/delta/delta_customers_initial_eg1.png)

And, you want to make the following updates to the table:

![Customer table updates](/data-engineering/gems/source-target/file/img/delta/delta_customers_updates_eg1.png)
...
<Info>
  ...
  lakehouses easier](https://www.prophecy.io/blog/prophecy-with-delta-making-data-lakehouse-easier).
</Info>
