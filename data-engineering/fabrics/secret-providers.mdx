---
title: Secrets
description: Store sensitive values in secret management systems
---
...
A _secret_ is sensitive information—such as an API key, password, or encryption key—that you want to use in a pipeline without exposing its value. Prophecy integrates with external [secret management systems](#secret-providers) to save those secrets securely, including, Databricks Secrets, HashiCorp Vault, and Spark environment variables.

To integrate secrets in Prophecy, you connect to these systems directly in your fabric. This ensures that the compute attached to that fabric can access the secret at runtime while keeping the value hidden from users. By linking secrets to fabrics, you can safely pass credentials and keys into Spark pipelines without hardcoding them.
...
![use_secret](/data-engineering/fabrics/img/Use_secret.png)

<Info>Typing credentials directly into fields triggers a warning diagnostic.</Info>

<Tip>
  If you want to populate secrets dynamically in your pipeline, you can set up secrets in [pipeline
  configurations](/data-engineering/development/pipelines/secrets-configs).
</Tip>
...
<Note>
  If your organization uses another system, you can fetch secrets by calling its API from a
  [Script](/data-engineering/gems/custom/script) gem, which runs PySpark code.
</Note>
...
<Info>
  If you are using a free trial Databricks fabric, you can use Databricks as the secret provider.
  Your secrets will be automatically cleaned up after the trial expires. While Prophecy assigns a
  separate scope to each trial fabric, it is not recommended to use your production data tools for
  trials.
</Info>
...
#### Mount paths with slashes

If your root mount name contains one or more slashes (`/`), you must provide the allowed secret path with a double slash (`//`) after the root mount.

**Example 1: Root mount with slashes**

```
secret/apps/team-a          (root mount)
└── db-config
    └── prod
```

Allowed secret path: `secret/apps/team-a//db-config/prod`

Note the double slash (`//`) after `team-a`.

**Example 2: Root mount without slashes**
...
Allowed secret path: `secret/app1/db-config/prod`

No double slash is needed when the root mount does not contain slashes.
...
<Note>This method does not support refreshing or fetching secrets.</Note>
