...
To run models projects on Google BigQuery, you need to create a fabric with a BigQuery connection.

<Warning>
  SQL fabrics are for **models only**. Create a [Prophecy
  fabric](/data-analysis/environment/fabrics/prophecy-fabrics) with a BigQuery warehouse connection
  to run pipelines.
</Warning>
