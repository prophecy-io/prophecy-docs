...
<Info>
  For more detailed information, see the [dbt reference
  documentation](https://docs.getdbt.com/reference/references-overview).
</Info>

## SQL Query

Add a custom SQL query at the end of your target model using the Databricks SQL dialect. This allows you to apply a final transformation step, which can be useful if you're importing an existing codebase and need to add conditions or filters to the final output. Custom queries support Jinja, dbt templating, and [variable](/data-engineering/development/models/configuration) usage for your last-mile data processing.
...
For a complete guide to defining how to write target tables, visit [Write Options](/data-analytics/gems/source-target/table/write-options).

## Data Tests

A data test is an assertion you define about a dataset in your project. Data tests are run on target models to ensure the quality and integrity of the final data that gets written to the warehouse. Learn how to build tests in [Test definitions](/data-analytics/development/tests/use-model-tests).
