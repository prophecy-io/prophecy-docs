...
We recommend using Selective sampling mode for all Databricks users. It is faster and more powerful than the default mode. Selective sampling is also important when working with [Databricks UC standard clusters](/data-engineering/fabrics/databricks/UCShared), as other sampling modes only generate data samples on edge nodes for this access mode.
