...
description: Learn about streaming data running on Spark Structured Streaming
---

<Warning>
  Prophecy no longer provides support for streaming pipelines. Please switch to batch pipelines for
  continued support.
</Warning>
...
Streaming [pipelines](/data-engineering/development/pipelines/pipelines) work differently from batch pipelines:
...
![How to Create a Streaming pipeline](/data-engineering/development/spark-streaming/img/create-streaming-pipeline.png)
...
4. To deploy the pipeline on Databricks, users can follow the same process described [here](/data-engineering/orchestration/databricks-jobs). A scheduled job will check if the Streaming pipeline is running every X minutes. If the pipeline is not running, the job will attempt to start it.
