...
![External_tags_list](/data-engineering/ci-cd/deployment/img/external_release_tag_list.png)
...
1. From `...` in the top right corner, select **(1) Deploy**. This opens the Deploy dialog.
   ...
   ![Deploy button](/data-engineering/ci-cd/deployment/img/external_release_tag_deploy.png)
...
2. Select a release version you wish to deploy by using the **(1) Choose a release** dropdown. Once you select a version, the table below shows the jobs that are going to be modified (there might not be any jobs). Click **(2) Deploy** to start the deployment.
   ...
   ![Deploy start](/data-engineering/ci-cd/deployment/img/external_release_tag_deploy_start.png)

   <Info>

   If you have enabled [Selective Job Deployment](./deployment.mdx#deployment-modes), then you can pick the jobs you wish to deploy. Additionally, you have the option to override the fabrics for these jobs. Job selection **is not required** to deploy the release.
...
![Add dependency](/data-engineering/ci-cd/deployment/img/external_release_tag_dependency.png)
...
![Add another dependency](/data-engineering/ci-cd/deployment/img/external_release_tag_create_dependency.png)
