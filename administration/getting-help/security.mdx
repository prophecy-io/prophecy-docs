...
<Info>
  Read more details on Prophecy's security and compliance posture at our [Security
  Portal](https://security.prophecy.io/).
</Info>
...
- Prophecy **does not** store or send your data to any third-party large language model (LLM) providers. Instead, Prophecy uses rich metadata to construct its [knowledge graph](/data-analysis/ai/knowledge-graph/knowledge-graph). As a result, Prophecy can interface with LLM providers while keeping your data private.
...
<Note>Using an LLM with Prophecy is optional.</Note>
