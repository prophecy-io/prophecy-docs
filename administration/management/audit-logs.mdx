...
---
{/*
  DRAFT: tracks the audit pipeline migration to Prophecy's unified telemetry system (feature flag
  telemetry.audits.enabled). As of 2026-09-09 this flag is off by default and the migration is not GA yet —
  hold this page until it is. Open items to confirm with engineering before publishing:
  1. Whether the "Sync data to S3" flow below (customer-controlled SIEM export bucket) still applies as a
     manual step under the new pipeline, or is superseded/automated.
  2. Whether the audit event reference tables below (GraphQL-query-shaped) still match what the new
     pipeline actually captures — newer event types (e.g. access-token create/delete) may not be reflected.
  3. What, if anything, logs audit events today for a default-config customer while the flag is off, given
     the old Postgres-backed audit path was removed from the codebase.
*/}
...
Audit logs are written as Parquet files through the same unified telemetry pipeline that captures product
usage metrics, and are queryable using AWS Athena.

<Note>
  AWS Athena is the AWS query service used to query these Parquet files — it's unrelated to any internal
  Prophecy service that happens to share the name.
</Note>

{/* DRAFT: re-verify this section's event catalog against the new audit library before publishing — see note 2 above */}
...
{/* DRAFT: confirm this manual sync flow still applies unchanged under the new pipeline before publishing — see note 1 above */}
