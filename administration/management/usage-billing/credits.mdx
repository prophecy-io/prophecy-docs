...
| **AI Requests**   | Each time you click on Copilot or interact with an Agent | 0.04 / request `TBD — confirm whether this moves to token-based billing (e.g. 5 tokens = expression, 15 tokens = description), and whether that replaces or supplements this flat rate` |
| **Compute**       | Processing required when running a gem or pipeline       | `TBD — reconciling 3 credits/CPU hour (current) vs. 0.12 credits/hour (proposed). May reflect a metering model change (sandbox-only vs. full CPU accounting) rather than a straight rate change.` |
