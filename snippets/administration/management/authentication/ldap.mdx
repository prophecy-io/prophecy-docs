...
- Group mapping can be enabled using [group-to-team mapping](/data-engineering/administration/management/authentication/group-team-mapping). To automatically create new teams in Prophecy via [group mappings](/data-engineering/administration/management/authentication/group-team-mapping), set the `ENABLE_AUTO_TEAM_CREATION` flag to `true` in your Prophecy deployment.
