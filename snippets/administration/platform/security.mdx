...
- Prophecy **does not** store or send your data to any third-party large language model (LLM) providers. Instead, Prophecy uses rich metadata to construct its [knowledge graph](/knowledge-graph). As a result, Prophecy can interface with LLM providers while keeping your data private.
