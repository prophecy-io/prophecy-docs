...
For more information about AI model support, visit [Prophecy AI](/data-analytics/ai/llm-providers).

## Users and sign-on

Express Edition supports up to 20 users maximum with a cap of five simultaneous users.

Learn how to add teams and users in [Team and user provisioning](/data-engineering/administration/management/users/team-user-provisioning).

### Optional: Authentication

Express Edition supports Single Sign-On (SSO) integration with Okta. Learn more about setting up Okta in our [SAML documentation](/data-analytics/administration/management/authentication/saml#set-up-saml).
...
- Create a [Prophecy fabric](/data-analytics/environment/fabrics/prophecy-fabrics). You can only create fabrics for teams where you are the [team admin](/data-analytics/administration/management/users/access/role-based-access).
- To add SQL warehouse connection in the fabric, set up a [Databricks connection](/data-analytics/environment/connections/databricks).
- To add data ingress/egress connections in the fabric, [follow the instructions](/data-analytics/environment/connections/connections) for each type of connection.
