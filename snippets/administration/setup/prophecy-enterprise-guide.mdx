Prophecy [Enterprise Edition](/data-engineering/administration/platform/editions) is designed for organizations that need robust security, scalability, and customization. It provides advanced features such as SCIM-based user management, additional compute options, and secure networking configuration.
...
- **LLM**: Enterprise Edition requires a customer-provided LLM endpoint to power [Prophecy AI](/data-analytics/ai/llm-providers).
...
- [Prophecy fabrics](/data-analytics/environment/fabrics/prophecy-fabrics): Use Prophecy Automate with an external SQL warehouse to run pipelines.
- [Spark fabrics](/data-engineering/fabrics/spark-provider/databricks/databricks): Use external Spark clusters to run pipelines.
