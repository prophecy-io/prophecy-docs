...
The [**Condition** gem](/data-analysis/gems/custom/conditional) routes rows to one of multiple outputs based on user-defined conditions. Each row is evaluated in order and routed to the first matching output.
...
Project parameters now support concrete types. Values are interpreted based on their content rather than treated as literal strings.
