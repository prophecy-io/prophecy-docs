...
You can now use the Prophecy Agent to [generate pipeline documentation](/data-analytics/development/ai-agent/ai-documentation.mdanalysts). Instead of writing documentation manually, this lets you create detailed specifications automatically.
...
- When using EMR [SAML authentication](/data-engineering/fabrics/emr/emr-saml) with Okta, you will see updated job size [configurations](/data-engineering/fabrics/emr/emr-job-sizes) in EMR fabrics.
...
- The [Amazon EMR Spark fabric](/data-engineering/fabrics/emr/emr) now [accepts SAML](/data-engineering/fabrics/emr/emr-saml) as an authentication method, using Okta as the identity provider (Added in 4.1.3.3).
...
PySpark project pipelines can now run on Databricks serverless compute. See supported features and limitations in [Databricks serverless compute for PySpark](/data-engineering/fabrics/databricks/databricks-serverless).
...
- You can now merge from the default branch (e.g., `main`) into another branch. Previously, the merge UI only allowed merging into the default branch. This enhancement is available in both the [Git workflow dialog](/data-engineering/ci-cd/git/git-workflow) within the project editor and the Commits tab on the [project metadata](/projects#project-metadata) page.

## Catalog table improvements

The [Catalog Table](/data-engineering/delta-table/) Target gem includes the following enhancements. Update the `ProphecySparkBasicsPython` package to version 0.2.47 to access these updates.
