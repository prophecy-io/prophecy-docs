<Update label="Prophecy 4.2.4" description="December 26, 2025" tags={["Data Engineering"]}>
...
<Update label="Prophecy 4.2.3.2" description="December 17, 2025" tags={["Data Analysis"]}>

## On-demand pipeline compilation

This feature lets you make multiple pipeline changes without triggering pipeline compilation after each edit. When ready, you can compile on-demand using the **Compile** button. This reduces latency when developing large pipelines and projects.

To use this feature, have your administrator or your Prophecy contact set `EDITOR_ON_DEMAND_COMPILATION: 'true'` in your Prophecy deployment configuration.

</Update>

<Update label="Prophecy 4.2.3.1" description="December 12, 2025" tags={["Data Analysis"]}>

## Data test API

Run data tests programmatically using the new [API](/api-reference/pipelines/run-data-tests). Execute column tests, table tests, and project tests together from a single API call. Trigger model tests from a separate API call.

</Update>

<Update label="Prophecy 4.2.3" description="December 8, 2025" tags={["Data Analysis"]}>

## Data tests compatible with pipelines

Prophecy now supports running data tests on Prophecy fabrics. (Previously, data tests were only supported on SQL fabrics.)

Learn more in [Table tests](/data-analysis/development/tests/table-tests) and [Project tests](/data-analysis/development/tests/project-tests).

## Schema validation for JSON and XML files

Prophecy lets you enable schema validation for [JSON](/data-analysis/gems/source-target/file/file-types/json) and [XML](/data-analysis/gems/source-target/file/file-types/xml) files in a Source gem. Use schema validation to ensure files conform to a predefined structure before ingesting the data. Schema validation works for all file storage connections.

## Knowledge graph scope

The [knowledge graph](/data-analysis/ai/knowledge-graph/knowledge-graph) indexer now scans all connections in a fabric, not just the SQL warehouse connection. This way, the Agent can access all data from the attached fabric.

## Charts improvements

Now, when you configure a chart in a [Prophecy App](/data-analysis/collaboration/prophecy-apps/apps) or in the [Data Explorer](/data-analysis/development/runs/data-explorer/data-explorer), you can change the colors used in the chart.

<Frame>
  ![Charts color picker](/data-analysis/development/runs/data-explorer/charts/img/chart-colors.png)
</Frame>

</Update>

<Update label="Prophecy 4.2.3" description="December 8, 2025" tags={["Data Engineering"]}>

## Selective sampling mode

There have been several improvements to [selective sampling](/data-engineering/development/runs/data-sampling#selective-sampling):

- Prophecy can now display the total [record count](/data-engineering/development/runs/data-sampling#record-counts) of selected gems.
- You can now use selective data sampling mode when running pipelines on [Livy](/data-engineering/fabrics/spark-provider/livy) clusters.
...
<Update label="Prophecy 4.2.2.2" description="November 25, 2025" tags={["Data Analysis"]}>

## Fabric APIs

We've exposed many [new endpoints](/api-reference/introduction) for creating and managing [fabrics](/data-analysis/environment/fabrics/prophecy-fabrics). You can now create, update, and delete fabrics, as well as manage fabric connections and secrets, via REST API.
...
Create custom templates for AI pipeline documentation generation using [Copilot Template Language (CTL)](/data-analysis/ai/agent/documentation/ctl-reference). CTL extends Markdown with special markers that generate interactive components like question forms, visual pipeline diagrams, and dynamic content that automatically updates based on your pipeline structure.

## Azure Data Lake Storage (ADLS) Source and Target gems

Use Azure Data Lake Storage (ADLS) as a source and target in your pipelines with the [ADLS gem](/data-analysis/gems/source-target/file/adls). The gem supports reading and writing CSV, JSON, Parquet, XLSX, and XML files. Source gems can read from specific file paths or from a file arrival/change trigger. Target gems can write to specific file paths.

## Knowledge graph indexer

You can now schedule automated indexing for the [knowledge graph indexer](/data-analysis/ai/knowledge-graph/indexer) to run hourly, daily, or weekly. The schedule supports custom timezone configuration, with the default timezone matching your Prophecy access location.

You can also configure separate authentication credentials for the [knowledge graph indexer](/data-analysis/ai/knowledge-graph/indexer#add-separate-authentication-for-the-indexer), independent from pipeline execution credentials. Use Service Principal OAuth for production environments where credentials don't expire, or User OAuth for development scenarios.
...
You can now use the Prophecy Agent to [generate pipeline documentation](/data-analysis/ai/agent/documentation/documentation). Instead of writing documentation manually, this lets you create detailed specifications automatically.

## New gems

- [Regex](/data-analysis/gems/parse/regex): Enables pattern matching and text extraction using regular expressions.
- [GenerateRows](/data-analysis/gems/prepare/generate-rows): Create new rows of data using iterative expressions.
- [GCS Source and Target](/data-analysis/gems/source-target/file/gcs): Read from or write to Google Cloud Storage (GCS) buckets.
- [Salesforce Target](/data-analysis/gems/source-target/external-table/salesforce): Supports writing to Salesforce Objects (CRM Analytics datasets not supported for targets).

## Gem improvements

- The [Email gem](/data-analysis/gems/report/email) now supports a **Use column** option for several parameters, including **To**, **Cc**, **Bc**, **Subject**, and **Body**. When enabled, these fields can be dynamically populated from columns in your input dataset. Each row in your dataset can specify a different recipient and/or custom message while sending the same attachment to all recipients.

- The [RestAPI gem](/data-analysis/gems/custom/rest-api) has been updated in the following ways:
  - The gem now supports parsing JSON responses into multiple output columns.

  - There are two new authentication methods — **Basic Auth** and **Bearer Token** — that automatically send credentials to APIs using new [Prophecy secret types](/data-analysis/environment/secrets/secrets).
...
- We have added an option to the [Directory gem](/data-analysis/gems/custom/directory) that enables you to list all the sheet names in XLSX files detected in a specified directory.

- You can now use the [DynamicInput gem](/data-analysis/gems/custom/dynamic-input) to union data from files defined in a column of file paths. Schemas are matched based on column names.

## Automatic package upgrades

When configuring [Prophecy project dependencies](/data-analysis/development/extensibility/dependencies), you can now enable automatic package upgrades. When enabled, the dependency will automatically upgrade to the latest version available.
...
You can now enable automatic team creation when using Microsoft Entra ID (Azure Active Directory) [SSO](/data-analysis/administration/management/authentication/azure-ad#step-2-optional-enable-automatic-team-creation).

## Team-level Copilot

We've added the ability to [enable or disable Copilot](/data-analysis/administration/management/teams/teams) at the team level.
...
The [Pipeline gem](/data-analysis/production/scheduling/pipeline-trigger-gem) lets you trigger another pipeline from within the current pipeline. You can use trigger conditions, pass parameters, and receive metadata for each run. Each gem execution can trigger the target pipeline multiple times, running sequentially until all runs finish.

## Data processing gems

- [CountRecords](/data-analysis/gems/transform/count-records): Count the number of rows in a dataset using different methods.
- [Directory](/data-analysis/gems/custom/directory): List files and folders of a specified directory from a data ingress/egress connection.
- [DataMasking](/data-analysis/gems/prepare/masking): Obfuscate sensitive string data in one or more columns.
- [DataEncoderDecoder](/data-analysis/gems/transform/encoder-decoder): Encode or decode data in selected columns using a variety of techniques.
- [FindDuplicates](/data-analysis/gems/prepare/find-duplicates): Filters rows in a dataset based on how frequently they appear.
- [RecordID](/data-analysis/gems/prepare/record-id): Generate row-level IDs using UUIDs for randomly generated values or incremental IDs for sequential values.
- [DynamicInput](/data-analysis/gems/custom/dynamic-input): Run SQL queries that update automatically based on your incoming data.

## Connections and source/target gems

The following are new connections and source/target gems:

- [Fixed-width Source](/data-analysis/gems/source-target/file/file-types/fixed-width): Read fixed-width files from various file storage accessed from connections.
- [Azure Synapse Source](/data-analysis/gems/source-target/external-table/synapse): Using the new Azure Synapse [connection](/data-analysis/environment/connections/synapse), you can read data from Microsoft SQL Server hosted on Azure Synapse dedicated SQL pools.
- [Hana Source and Target](/data-analysis/gems/source-target/external-table/hana/hana): Using the new HANA connection, you can read and write to an SAP HANA database (Added in 4.1.3.3).

Additionally, the following enhancements have been made to existing gems:

- We've added a **File Encoding** property to [CSV Source and Target](/data-analysis/gems/source-target/file/file-types/csv) gems. This lets you decode CSV files when reading and encode CSV files when writing.
- [SFTP](/data-analysis/gems/source-target/file/sftp) and [S3](/data-analysis/gems/source-target/file/s3) Source gems have the following enhancements that allow you to (Added in 4.1.3.3):
...
We've added a feature that lets you divide your pipeline into logical sections using [visual containers](/data-analysis/development/studio/containers) for canvas organization.

## Version control improvements

You can now [move projects](/data-analysis/development/versioning/migrate-managed) hosted in a Prophecy-managed Git repository into an external repository without having to clone the project.

## Enhancements

- You can now enable or disable [auto-run](/data-analysis/collaboration/prophecy-apps/app-settings) for Prophecy Apps in the app settings.
- When you open a gem, you can now collapse the **Ports** panel of the gem configuration dialog.
- [Write strategies](/data-analysis/gems/source-target/table/write-options) for Table gems have been renamed. However, they are still mapped to the same dbt configs under the hood (Changed in 4.1.3.9).
...
Prophecy now supports running SQL transformations in your BigQuery warehouse. Learn how to configure a [BigQuery connection](/data-analysis/environment/connections/bigquery) in fabrics to leverage this capability.

## Spatial gems

- [Buffer](/data-analysis/gems/spatial/buffer): Take any polygon or line and expand or contract its boundaries.
- [FindNearest](/data-analysis/gems/spatial/nearest-point): Find the closest spatial point(s) between two datasets based on geographic distance.
- [Simplify](/data-analysis/gems/spatial/simplify): Reduce the number of vertices in polygons and polylines while preserving overall shape.
- [HeatMap](/data-analysis/gems/spatial/heatmap): Transform latitude and longitude point data into a spatial heatmap using hexagonal tiling.
- [PolyBuild](/data-analysis/gems/spatial/polybuild): Build spatial shapes from coordinate data by grouping and ordering points into either polygons or polylines.
- [SpatialMatch](/data-analysis/gems/spatial/spatial-match): Compare geometries and return only the pairs that have the spatial relationship you specify.

## Transformation gems

- [Pivot](/data-analysis/gems/transform/pivot): Reshape your data by transforming unique row values from one column into new columns.
- [Unpivot](/data-analysis/gems/transform/unpivot): Convert multiple columns into row-level key-value pairs to simplify columnar data.

## Connections and source/target gems

- Connect to [Salesforce](/data-analysis/gems/source-target/external-table/salesforce) and read data into your pipelines.
- Prophecy now supports uploading [Parquet](/data-analysis/gems/source-target/file/file-types/parquet) files to Databricks connections.

## File arrival trigger

Pipeline schedules can now be triggered by [file arrivals or changes](/data-analysis/production/scheduling/triggers#file-arrival-or-change-trigger) in a specified SFTP or S3 location.

## Stored procedures

You can now define and call [stored procedures](/data-analysis/gems/custom/stored-procedure) within SQL projects running on BigQuery. This is particularly useful when migrating workflows from other systems.
...
- You can now overwrite a Table gem using the [File Upload component](/data-analysis/collaboration/prophecy-apps/app-components#file-upload) in a Prophecy App. Previously, you could only overwrite Source gems. (Added in 4.1.2.2).
...
Databricks connections in Prophecy fabrics now accept [service principal](https://docs.databricks.com/aws/en/admin/users-groups/service-principals) credentials for OAuth. Now you can deploy scheduled pipelines using the service principal credentials. To learn more, visit the [Databricks connection](/data-analysis/environment/connections/databricks) documentation.
...
The new [Power BI connection](/data-analysis/environment/connections/power-bi) and [PowerBIWrite gem](/data-analysis/gems/report/power-bi) allow you to you publish pipeline results directly to Power BI tables (Added in 4.1.1.5).
...
## ProphecyDatabricksSqlSpatial v0.0.1
