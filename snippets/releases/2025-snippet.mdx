...
Learn more in [Table tests](/data-analytics/development/tests/table-tests) and [Project tests](/data-analytics/development/tests/project-tests).

## Schema validation for JSON and XML files

Prophecy lets you enable schema validation for [JSON](/data-analytics/gems/source-target/file/file-types/json) and [XML](/data-analytics/gems/source-target/file/file-types/xml) files in a Source gem. Use schema validation to ensure files conform to a predefined structure before ingesting the data. Schema validation works for all file storage connections.

## Knowledge graph scope

The [knowledge graph](/data-analytics/ai/knowledge-graph/knowledge-graph) indexer now scans all connections in a fabric, not just the SQL warehouse connection. This way, the Agent can access all data from the attached fabric.

## Charts improvements

Now, when you configure a chart in a [Prophecy App](/data-analytics/collaboration/prophecy-apps/apps) or in the [Data Explorer](/data-analytics/development/runs/data-explorer/data-explorer), you can change the colors used in the chart.

<Frame>
  ![Charts color picker](/data-analytics/development/runs/data-explorer/charts/img/chart-colors.png)
</Frame>
...
We've exposed many [new endpoints](/api-reference/introduction) for creating and managing [fabrics](/data-analytics/environment/fabrics/prophecy-fabrics). You can now create, update, and delete fabrics, as well as manage fabric connections and secrets, via REST API.
