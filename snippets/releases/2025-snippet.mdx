...
We've added the ability to [enable or disable Copilot](/data-analytics/administration/teams) at the team level.
...
- More gems in Python/PySpark projects are now compatible with [Databricks UC clusters](/data-engineering/fabrics/databricks/UCShared) configured with standard (formerly shared) access mode. The table below shows the minimum package version required to enable compatibility.
