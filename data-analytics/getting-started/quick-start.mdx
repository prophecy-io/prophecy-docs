...
Take a few minutes to try out the Prophecy Agent for pipeline development. In this quick start, you'll learn how to use the Agent to explore data, build transformations, and create complete pipelines through natural language conversations.
...
- A Prophecy fabric with a configured [Databricks SQL warehouse connection](/environment/connections/databricks).
...
<Note>
  The Agent can only read and transform Databricks Unity Catalog tables. It cannot read, write, or
...
<Note>
  Each change that the AI agent makes can be viewed in the [project version
...
## 4. Build your first transformation
...
| -------------------------------------- | -------------------------------------------------- | ------------------------------------------------------------------------------------------------------ |
| Agent can't find tables                | [Knowledge graph](/knowledge-graph) is out of date | Reindex your Databricks connection so the knowledge graph includes the table                           |
