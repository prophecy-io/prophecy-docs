---
title: Exploration
description: Find and preview data sources using the Prophecy Agent
---
...
Ask the agent to search for tables in your [primary SQL warehouse](/data-analytics/environment/fabrics/prophecy-fabrics). Based on your query, it returns:

- A short list of relevant datasets and their descriptions.
- A full list of datasets that match your criteria.

<Tip>
  You can add a dataset to the pipeline as a [Table gem](/data-analytics/gems/source-target/source-target) directly from the
  chat.
</Tip>
...
![Preview dataset UI](/data-analytics/ai/agent/img/agent-explore.png)
...
![Data sample response](/data-analytics/ai/agent/img/agent-sql-logs-sample.png)
...
![Chart generation response](/data-analytics/ai/agent/img/agent-sql-logs-charts.png)
...
<Info>To learn more about data visualization, see [Charts](/data-analytics/development/runs/data-explorer/charts/charts).</Info>
...
The Agent depends on a [knowledge graph](/data-analytics/ai/knowledge-graph/knowledge-graph) to retrieve metadata about components like datasets. If the agent does not recognize a table that you try to reference, it could be because the knowledge graph has not been indexed recently enough to capture the dataset.

For detailed instructions on indexing, see [Knowledge graph indexer](/data-analytics/ai/knowledge-graph/indexer).
