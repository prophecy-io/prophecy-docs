---
title: Exploration
description: Find data sources using the Prophecy Agent
---
...
<Tip>
  You can add a dataset to the pipeline as a [Table gem](/analysts/source-target) directly from the
  chat.
</Tip>
...
<Info>To learn more about data visualization, see [Charts](/analysts/charts).</Info>
...
The Agent depends on a [knowledge graph](/knowledge-graph) to retrieve metadata about components like datasets. If the agent does not recognize a table that you try to reference, it could be because the knowledge graph has not been indexed recently enough to capture the dataset.
