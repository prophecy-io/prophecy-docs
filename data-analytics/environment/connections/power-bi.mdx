...
| Feature                                                                    | Supported |
| -------------------------------------------------------------------------- | --------- |
| Read and write using [Source and Target gems](/data-analytics/gems/source-target/source-target)     | No        |
| Write data with a [PowerBIWrite gem](/data-analytics/gems/report/power-bi)             | Yes       |
...
Power BI connections are stored within [fabrics](/data-analytics/environment/fabrics/prophecy-fabrics), which are assigned to specific teams in Prophecy. Once a Power BI connection is added to a fabric, anyone on that team can use it to send data to Power BI from their pipelines. Everyone will inherit the permissions of the Microsoft Entra app used for connection setup.
