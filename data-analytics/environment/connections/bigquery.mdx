...
Prophecy supports BigQuery as both a SQL Warehouse connection and an Ingress/Egress connection. To learn more about these different connection types, visit [Prophecy fabrics](/data-analytics/environment/fabrics/prophecy-fabrics).
...
| Read and write data with a [Table gem](/data-analytics/gems/source-target/table/bigquery)   | Yes — SQL Warehouse Connection only |
| Read data with a [Source gem](/data-analytics/gems/source-target/external-table/bigquery)   | Yes                                 |
| Write data with a [Target gem](/data-analytics/gems/source-target/external-table/bigquery)  | Yes                                 |
...
| Dataset               | The default location for target tables and [temporary tables](/data-analytics/development/runs/execution#external-data-handling). <br />Requires write permissions.                                                                                                                                                                                                                      |
...
If no app registrations appear, an admin must configure an [OAuth app registration](/data-analytics/administration/management/cluster-admin-settings/oauth-setup).
...
1. Paste the full JSON content into a [Prophecy secret](/data-analytics/environment/secrets/secrets) as text. Binary upload is not supported.
