...
| Write data with a [TableauWrite gem](/data-analytics/gems/report/tableau)                    | Yes       |
...
| Tableau Token ([Secret required](/data-analytics/environment/secrets/secrets)) | Your Tableau personal access token                                    |
...
Tableau connections are stored within [fabrics](/data-analytics/environment/fabrics/prophecy-fabrics), which are assigned to specific teams in Prophecy. Once a Tableau connection is added to a fabric, anyone on that team can use it to send data to Tableau from their pipelines. Everyone will inherit the permissions of the user authenticated during connection setup.
