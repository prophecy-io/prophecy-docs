...
A **secret** is stored sensitive data such as passwords, API keys, or certificates. Prophecy includes a built-in secret manager to store secrets securely and allow [Prophecy Automate](/data-engineering/administration/platform/architecture) to access them at runtime. This prevents credentials from being hardcoded or shared in plain text.
...
Secrets are often used to hide credentials in [connections](/data-analytics/environment/connections/connections).
