...
When creating a Prophecy App, structure it using [pipeline parameters](/data-analytics/development/pipeline-params). Pipeline parameters allow applications to dynamically adjust values during execution. By adding [interactive components](/data-analytics/collaboration/prophecy-apps/app-components) to your apps, users can define parameter values while keeping the rest of the pipeline unchanged.
...
![Run Prophecy App results](/data-analytics/collaboration/prophecy-apps/img/prophecy-app-run.png)
...
![App pipeline](/data-analytics/collaboration/prophecy-apps/img/app-pipeline.png)

### Create a pipeline parameter

[Pipeline parameters](/data-analytics/development/pipeline-params) enable dynamic behavior in pipelines by allowing values to be set at runtime. In this case, these values can be driven by inputs from external sources, including user input through a Prophecy App.
...
1. Enter `3000007` as the default value to be used during [interactive pipeline runs](/data-analytics/development/runs/execution). (This is an arbitrary franchise ID.)
1. Click **Save**.

![Create pipeline parameter dialog](/data-analytics/collaboration/prophecy-apps/img/pipeline-parameter.png)
...
![Configure visual expression to use parameter](/data-analytics/collaboration/prophecy-apps/img/visual-expression-parameter.png)
...
![Prophecy App template](/data-analytics/collaboration/prophecy-apps/img/prophecy-app-template.png)
...
To explore advanced configuration options and scheduling capabilities, refer to the documentation on [running apps](/data-analytics/collaboration/prophecy-apps/run-apps).
...
To address your specific business requirements, leverage more complex [components](/data-analytics/collaboration/prophecy-apps/app-components) to construct robust Prophecy Apps.
