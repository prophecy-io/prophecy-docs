...
Use the Pivot gem to convert your table from long format to wide format.

- Select a column whose unique values will become new columns in the output.
- Each unique value in this column creates a new column in your result table.

This helps you summarize and analyze data more easily, letting you see multiple categories side by side.
...
<Info>
  When using the `CONCAT` aggregation method, the **Separator** parameter specifies the character or
  string used to separate the values that are being concatenated together. Common separators include
  commas, semicolons, and pipes.
</Info>
