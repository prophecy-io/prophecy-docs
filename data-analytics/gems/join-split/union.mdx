...
All input tables must have **identical schemas** (matching column names and data types). If table columns have different orders or a different number of columns, use the [UnionByName](/data-analytics/gems/join-split/union-by-name) gem.
...
<Tip>Order `103` and `104` are duplicate records.</Tip>
