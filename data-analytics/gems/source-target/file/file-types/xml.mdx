...
<Note>
  To use schema validation, you need an XSD schema file in the same directory as your source XML
  file. If your schema uses `xs:include` or `xs:import` to reference external schemas, those
  referenced schema files must also be in the same directory.
</Note>
...
The following example shows a sample XML file and a corresponding XSD schema file that would validate successfully.
