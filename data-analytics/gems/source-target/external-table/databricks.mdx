...
This page describes how to use Databricks external Source and Target gems to read from or write to tables. Only use an external Source and Target gem when Databricks is not the configured [SQL warehouse connection](docs/administration/fabrics/prophecy-fabrics/create-fabric.md#connections). Otherwise, use the [Table gem](/data-analytics/gems/source-target/table/bigquery).

<Info>
  If you're working with file types like CSV or Parquet from Databricks file storage, see [File
  types](/data-analytics/gems/source-target/file) for guidance. This page focuses only on catalog tables.
</Info>
...
- **Source/Target location**: Choose the [connection](/data-analytics/environment/connections/connections) and define the location where you will read/write tables in Databricks.
