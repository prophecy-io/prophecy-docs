---
title: Parameters
description: Add variables to your projects
---
...
- Enabling user-entered fields in [Prophecy Apps](/data-analytics/collaboration/prophecy-apps/apps).
- Keeping values consistent throughout the pipeline.
- Configuring pipelines for multiple execution environments (such as development or production). Use app configs or the [Deploy Project API](/api-reference/endpoint/deploy-project#pipeline-configuration-structure) to do so.

Once you create pipeline parameters, they are available as _configuration variables_ in [gems](/data-analytics/gems/gems) wherever you can select a visual expression. You can also reference pipeline parameters using Jinja syntax in code expressions.
...
<Info>
  If a pipeline parameter and a project configuration variable share the same name, the value of the
  pipeline parameter takes precedence within that pipeline.
</Info>
...
1. Add any [gem](/data-analytics/gems/) that uses expressions, such as a [Filter](/data-analytics/gems/prepare/filter) or [Reformat](/data-analytics/gems/prepare/reformat) gem.
...
Parameters are useful in both [creating](/data-analytics/collaboration/prophecy-apps/app-creation) and [consuming](/data-analytics/collaboration/prophecy-apps/run-apps) Prophecy applications. To use pipeline parameters in a Prophecy App, add an input field and select the pipeline parameter for **Configuration field**.
...
1. Add a [Reformat gem](/data-analytics/gems/prepare/reformat).
...
- Keep sensitive values (such as API keys) in [secrets](/data-analytics/environment/secrets/secrets) rather than passing them as plain parameters.
