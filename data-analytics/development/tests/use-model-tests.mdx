...
To create your own test definitions, you need to understand these SQL concepts:

- **Select statements**: How to query data from tables
- **Where clauses**: How to filter rows based on conditions
- **Aggregations**: Functions like `COUNT()`, `SUM()`, `MAX()` that calculate values
- **Joins**: How to combine data from multiple tables (for tests that compare models)
- **CTEs (Common Table Expressions)**: The `WITH` clause that lets you break complex queries into named parts
...
1. The first CTE (`a`) counts rows in your model.
...
1. In the **Parameters** table, add the parameters for the test. Each test has a default `model` parameter. When you apply a test to a table or model, Prophecy automatically sets this parameter to that entity's name. The `not_constant` test also requires a reference to a column in the model. Thus, you need to add another parameter called `column_name` of type `column`.
...
When you add a test definition to a table or model, you set the test parameters. In addition, you can also set the following advanced settings for all tests:
