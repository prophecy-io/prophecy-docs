...
Next, the project is deployed to different environments. The deployment process builds the project in each target environment ([fabric](/data-analytics/environment/fabrics/prophecy-fabrics)) and enables pipeline [schedules](/data-analytics/production/scheduling/scheduling) for each fabric.
...
To publish a project to a fabric, that fabric must be [assigned to one of your teams](/data-analytics/administration/management/users/access/team-based-access). This ensures that only authorized users can deploy projects to specific environments. For example:
...
If your production fabric uses Databricks connections, [consider using a service principal](/data-analytics/environment/connections/databricks#authentication-methods) for authentication. This helps scheduled pipelines in published projects run reliably.
...
| Select the fabric(s) to publish | Prophecy creates a new project deployment for each fabric. If a deployment already exists, it is updated. To see your deployments, review the [Monitoring](/data-analytics/production/monitoring) page. |
...
Prophecy lets multiple users work on the same project [simultaneously](/data-analytics/development/versioning/collaboration-modes). If you try to publish the project while multiple people are working on it, you can **request to publish** the project. This way, all collaborators can be aware of the publication as they work on the project and approve or veto the action.

## Monitor deployed projects

All of your teams' published projects will appear in the [Observability](/data-analytics/production/monitoring) interface in Prophecy. This lets you see information like:
...
![Publish logs](/data-analytics/production/img/publish-logs.png)
